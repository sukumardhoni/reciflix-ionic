angular.module('recipesApp')
	...
	.controller('AppCtrl', function ($scope, SearchedRecipes, $stateParams, $ionicLoading, $timeout, Authentication, $state, $ionicPopup, User, $localStorage, $http, $rootScope, $ionicHistory, $ionicDeploy, $ionicPlatform, $cordovaGooglePlus) {
		...
		$scope.signout = function () {
			...
			User.Signout.clear(function (res) {
				if (res.type === false) {
					...
					$ionicLoading.hide();
					if ($localStorage.googleAuthKey) {
						$cordovaGooglePlus.logout(
							function (msg) {
								//alert(msg); // do something useful instead of alerting
								console.log('Successfully logout google user');
								$state.go('landing');
							}
						);
					} else if (!$localStorage.googleAuthKey) {
						openFB.logout(
							function (response) {
								$state.go('landing');
							})
