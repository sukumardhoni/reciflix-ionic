angular.module('recipesApp')
	...
	.controller('landingCtrl', function ($scope, $state, User, $ionicLoading, $rootScope, Authentication, $localStorage, $http, $timeout, $ionicHistory, $cordovaOauth, $stateParams, $ionicDeploy, $ionicPlatform, $cordovaGooglePlus) {
					...
					if (response.status === 'connected') {
						openFB.api({
							...
							success: function (user) {
								...
								$localStorage.googleAuthKey = false;
								...
								if (user.email) {
									...
									$scope.errMsg = 'This seems to be Facebook login error. We will look into it and let you know';
									$ionicLoading.hide();
									openFB.logout(
										function (response) {
		...
		$scope.getDataProfile = function () {
			...
			$http({
				...
				.error(function (data, status) {
					$scope.errMsg = 'This seems to be Google login error. We will look into it and let you know';
				});
		};
		...
		$scope.googleAuth = function () {
			...
			$localStorage.googleAuthKey = true;
			$cordovaGooglePlus.login({})
				.then(function (user) {
					console.log('Succesfully login : ' + JSON.stringify(user));
					...
					$localStorage.picture = user.imageUrl;
					User.Signup.create($scope.gUser, function (res) {
						if (res.type === false) {
							if (res.user) {
								$scope.populateUserLocally(res.user);
